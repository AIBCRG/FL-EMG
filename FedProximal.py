...
# Function to perform federated proximal
# Federated Proximal uses a regularization term to keep local updates closer to the global model
def federated_proximal(global_weights, local_weights_list, mu):
    new_weights = []
    for global_weight, *local_weights in zip(global_weights, *local_weights_list):
        # Calculate the weighted average of local weights with regularization
        local_update = np.mean(local_weights, axis=0) - global_weight
        proximal_update = global_weight + (1 - mu) * local_update
...
def main():
    ...
    batch_size = 32
    mu = 0.1  # Regularization parameter for Federated Proximal
    ...
    for round_num in range(num_rounds):
        ...
        # Federated Proximal
        new_weights = federated_proximal(global_model.get_weights(), local_weights, mu)
