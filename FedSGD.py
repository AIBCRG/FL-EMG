...
# Function to perform federated stochastic gradient descent
def federated_sgd(global_weights, local_weights_list, learning_rate):
    new_weights = []
    for global_weight, *local_weights in zip(global_weights, *local_weights_list):
        avg_local_weight = np.mean(local_weights, axis=0)
        new_weight = global_weight - learning_rate * (global_weight - avg_local_weight)
...
def main():
    ...
    batch_size = 32
    learning_rate = 1  # Learning rate for FedSGD
    ...
    for round_num in range(num_rounds):
        ...
        # Federated SGD
        new_weights = federated_sgd(global_model.get_weights(), local_weights, learning_rate)
